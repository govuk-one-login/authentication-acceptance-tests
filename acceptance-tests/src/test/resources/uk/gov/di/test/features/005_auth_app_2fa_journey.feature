...
Feature: Authentication App Journeys
  ...
  Scenario: User successfully registers with auth app 2FA and login with 2fa-on
    ...
    When the user returns from the stub relying party with default options and is taken to the "Create your GOV.UK One Login or sign in" page
  ...
  Scenario: User signs in auth app without 2FA, then uplifts
    ...
    Then the user returns from the stub relying party with options: [2fa-on,authenticated-2,authenticated-level] and is taken to the "Enter a security code to continue" page
