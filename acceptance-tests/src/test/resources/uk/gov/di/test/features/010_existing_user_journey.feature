...
Feature: Login Journey
  ...
  Scenario: Existing user logs in without 2FA then uplift with 2FA
    ...
    When the user enters their password
    Then the user is returned to the service
    When the user returns from the stub relying party with options: [2fa-on,authenticated-2,authenticated-level] and is taken to the "Enter a security code to continue" page
