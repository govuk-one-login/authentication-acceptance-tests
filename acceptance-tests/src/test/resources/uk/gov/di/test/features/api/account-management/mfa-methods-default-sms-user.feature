...
Feature: SMS MFA User manages their MFA methods via the Method Management API
  ...
  Scenario: Authenticated User successfully adds a Backup Auth App
  ...
  Scenario: Authenticated User successfully Adds a Backup SMS
    And the User does not have a Backup MFA method
  ...
  Scenario: Prevented from changing Default method to Auth App when Backup is Auth App
    ...
    When the User requests to add a backup MFA Auth App with Default Auth App
    Then the User's back up MFA Auth App is not updated
