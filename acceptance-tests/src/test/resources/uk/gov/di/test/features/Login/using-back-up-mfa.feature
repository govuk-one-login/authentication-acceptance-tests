...
@AUT-1416
  Scenario: User with SMS as default MFA attempts authentication using a backup SMS number
...
@AUT-4008
  ...
  Scenario: User with Auth App as default MFA attempts alternative MFA journey but submits without choosing a method
    Given a Migrated User with an Auth App Default MFA
    And the User is Authenticated
    And the User does not have a Backup MFA method
    When the User adds "07700900111" as their SMS Backup MFA
